...
def match_one_line(line):
    ...
    result["j_match"] = re.match(r"\s*JOINT\s+(\w+:?\w+)", line)
    result["f_match"] = re.match(r"\s*Frames:\s+(\d+)", line)
    result["ft_match"] = re.match(r"\s*Frame Time:\s+([\d\.]+)", line)
    result["d_match"] = line.strip().split()
    return result


def read_bvh(file_name: str, start=None, end=None,order='zxy', world=False, need_quater=True):
    """
...
def save_bvh(file_name, anim, names=None, frame_time=1.0/24.0, order='zxy', positions=False, orients=True, mask=None,
